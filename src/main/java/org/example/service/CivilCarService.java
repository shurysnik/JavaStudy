...
public class CivilCarService extends VehicleService<CivilCar> {
    private static CivilCarService instance;

    private CivilCarService(CrudRepository<CivilCar> repository) {
        super(repository);
    }

    public static CivilCarService getInstance() {
        if (instance == null) {
            instance = new CivilCarService(CivilCarRepository.getInstance());
        }
        return instance;
    }

    @Override
    public CivilCar create() {
